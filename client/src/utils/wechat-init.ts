...
const wechatInit = async (
  params: wx.ConfigOptions & { onReady?: () => void; onError?: () => void }
) => {
  ...
  wx.config({
    ...
    signature, // 必填，签名
    jsApiList, // 必填，需要使用的JS接口列表，具体可以查看JS接口列表：<https://developers.weixin.qq.com/doc/offiaccount/OA_Web_Apps/JS-SDK.html#63>
  });
  // 通过ready接口处理成功验证
  wx.ready(function () {
